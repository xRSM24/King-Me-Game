...
The Android project is already in `android/`. Package name **`app.kingme.climb`**. Target SDK **35**. You do **not** need a Mac. You do need Android Studio (or the SDK) on your computer, a Play Console account, and a **public** privacy URL.
...
In Android Studio: **Build → Generate Signed App Bundle**. First time, create a keystore (`kingme.jks`) and keep that file and its passwords somewhere safe — losing it means you cannot update the listing. On the first upload, enroll in **Play App Signing** (default).
